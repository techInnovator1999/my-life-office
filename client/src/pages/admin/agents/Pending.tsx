...
import { getPendingCrmAgents, approveAgent } from '@/services/agentService'
...
export function Pending() {
  ...
  const fetchPendingAgents = async () => {
    try {
      setIsLoading(true)
      setError(null)
      const response = await getPendingCrmAgents(page, limit)
      setAgents(response.data)
      setTotal(response.total || response.data.length)
      setHasNextPage(response.hasNextPage)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to fetch pending agents')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleConfirmApprove = async () => {
    ...
    try {
      setApprovingId(agentToApprove.id)
      await approveAgent(agentToApprove.id)
  ...
  return (
    <>
      {/* Confirmation Modal */}
      {showConfirmModal && agentToApprove && (
        <>
          {/* Backdrop */}
          <div
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div
              className="bg-white dark:bg-surface-dark rounded-lg shadow-xl border border-neutral-200 dark:border-slate-700 w-full max-w-md"
            ...
            >
              ...
              <div className="px-6 py-4">
                <p className="text-sm text-text-muted dark:text-text-muted-dark">
                  ...
                  <span className="font-medium text-text-main dark:text-white">
                    {formatFullName(agentToApprove.firstName, agentToApprove.lastName)}
                  </span>
                  ? This action will grant them full access to the system.
                </p>
              </div>
              ...
              <div className="px-6 py-4 border-t border-neutral-200 dark:border-slate-700 flex items-center justify-end gap-3">
                <button
                  onClick={handleCancelApprove}
                  disabled={approvingId === agentToApprove.id}
                ...
                <button
                  ...
                  disabled={approvingId === agentToApprove.id}
                  className="px-4 py-2 text-sm font-medium bg-primary text-white rounded-md shadow-sm hover:bg-primary-hover hover:shadow-md active:bg-primary-800 active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {approvingId === agentToApprove.id ? (
      ...
      <div>
        ...
        <p className="text-text-muted dark:text-text-muted-dark mt-1">Review and approve pending agent registrations</p>
      </div>
      ...
      <div className="bg-white dark:bg-surface-dark rounded-xl shadow-sm border border-neutral-200 dark:border-slate-700 overflow-hidden">
        {isLoading ? (
          <div className="p-8 text-center">
            ...
            <p className="text-text-muted dark:text-text-muted-dark mt-2">Loading pending agents...</p>
          </div>
        ) : agents.length === 0 ? (
          <div className="p-8 text-center">
            <p className="text-text-muted dark:text-text-muted-dark">No pending agents found</p>
          </div>
        ) : (
          <>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 text-xs font-medium bg-white dark:bg-surface-dark text-gray-700 dark:text-gray-300 border border-neutral-200 dark:border-slate-700 rounded-md hover:bg-neutral-50 dark:hover:bg-slate-700 transition-colors"
                          >
                            View
                          </button>
                        </div>
            ...
            {total > limit && (
              <div className="px-6 py-4 border-t border-neutral-200 dark:border-slate-700 flex items-center justify-between">
                <div className="text-sm text-text-muted dark:text-text-muted-dark">
                  Showing {((page - 1) * limit) + 1} to {Math.min(page * limit, total)} of {total} pending agents
                </div>
                <div className="flex items-center gap-2">
                  <button
